...
def benchmark_function(func, *args, warmup=10, repeat=100, **kwargs):
    """Benchmark a function with warmup and repeated runs.
    ...
    Returns:
        dict: Statistics including mean, std, min, max time in ms
    """
    # Warmup
...
def attention_paddle(q, k, v, dropout_p=0.0, causal=True):
    """Paddle implementation of attention.
...
if __name__ == "__main__":
    ...
    # custom_attention returns [out, softmax_lse, p, rng_state]
    if isinstance(binding_out, list):
        binding_out = binding_out[0]
    
    paddle.device.cuda.synchronize()
    max_diff, mean_diff = check_tensor(binding_out, pd_out)
    
    print(f"[CHECK]  Binding_Attn\t  max_diff:{max_diff:.4f}  mean_diff:{mean_diff:.4f}" )
    # 找出所有差异较大的位置
    diff = (binding_out - pd_out).abs()
    large_diff_mask = diff > 0.5  # 找出差异 > 0.5 的位置
    large_diff_indices = paddle.nonzero(large_diff_mask.flatten())

    print(f"\n[DEBUG] Found {large_diff_indices.shape[0]} positions with diff > 0.5")

    # 分析前 10 个最大差异的位置
    topk_diff_values, topk_indices = paddle.topk(diff.flatten(), k=10)
    for i, (idx, val) in enumerate(zip(topk_indices, topk_diff_values)):
        idx = idx.item()
        val = val.item()

        # 解析位置 - 注意 tensor shape 是 [batch_size, seqlen, nheads, headdim]
        batch_idx = idx // (seqlen * nheads * headdim)
        remainder = idx % (seqlen * nheads * headdim)
        seq_idx = remainder // (nheads * headdim)
        remainder = remainder % (nheads * headdim)
        head_idx = remainder // headdim
        dim_idx = remainder % headdim

        binding_val = binding_out[batch_idx, seq_idx, head_idx, dim_idx].item()
        paddle_val = pd_out[batch_idx, seq_idx, head_idx, dim_idx].item()

        print(f"  Top {i+1}: batch={batch_idx}, head={head_idx}, seq={seq_idx}, dim={dim_idx} "
              f"| diff={val:.4f} | binding={binding_val:.4f}, paddle={paddle_val:.4f}")
