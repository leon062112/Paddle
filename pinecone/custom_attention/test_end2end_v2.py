...
# 使用 Paddle Native Attention 的完整 BERT 前向传播
def bert_fwd_paddle_native(mask):
    with paddle.no_grad():
        ...
        for layer in range(layer_num):
            ...
            # ------------------------------------------------------------- Attention start
            # 使用 Paddle Native Attention (ref_program)
            h = ref_program(q, k, v, is_causal)
...
# 使用 STOF Attention 的完整 BERT 前向传播
def bert_fwd_stof(mask):
    with paddle.no_grad():
        hidden_states = input_from_tensor
        for layer in range(layer_num):
            input_tensor = hidden_states
            ...
            # FFN: 使用 AP 融合算子：matmul + add + gelu
...
if __name__ == "__main__":
    ...
    # Paddle Native - 测试完整的 BERT 层 ---------------------------------------
    if test_Paddle:
        attention_type = "paddle_native_attention"
        
        for i in range(warmup_iters + running_iters):
            if i == warmup_iters:    
                t1_start = time_stamp_cudasync()

            bert_fwd_paddle_native(mask)
            paddle_output = transformer_output[-1]

        t1_end = time_stamp_cudasync()
        paddle_time = (t1_end - t1_start) * 1000 / running_iters   
        print("e2e {} | bs:{} | seq:{} | Paddle Native   : {:.3f} ms / iter".format(
            model_selection, batch_size, args.seq_len, paddle_time)) 
